# -*- coding: utf-8 -*-
""" nns/models/layers/disagreement_attention/intra_model/bidirectional_mixed_embedded """
...
__all__ = ['BidirectionalMixedEmbeddedDABlock']


class BidirectionalMixedEmbeddedDABlock(BaseDisagreementAttentionBlock):
    r"""
    Calculates the intra-class Bidirectional Mixed Embedded Disagreement Attention and returns the
    act1 with the computed attention
    ...
    attention = 2 (skip_conn + gating_signal + |gating_signal - skip_conn|)

    Usage:
        g = BiDirectionalMixedEmbeddedDABlock(320, 1024)
        g(act, gs)
    """
    ...
    def forward(self, act1: torch.Tensor, act2: torch.Tensor):
        ...
        wact1 = self.w1(act1)
        wact2 = self.w2(act2)
        attention = self.act_with_attention(2 * (wact1 + wact2 + torch.abs(wact2-wact1)))
