# -*- coding: utf-8 -*-
""" nns/mixins/managers/sdsv2_manager """
...
__all__ = ['SDSVModelMGRMixin2']


class SDSVModelMGRMixin2(BaseModelMGR):
    """
    ...
    def training(self):
        ...
        for epoch in range(start_epoch, self.epochs):
            ...
            tmp_epoch_train_loss_list = None
            ...
            with tqdm(total=self.n_train, desc=f'Epoch {epoch + 1}/{self.epochs}', unit='img',
                ...
                for batch in self.train_loader:
                    ...
                    batch_loss_list = []
                    tmp_epoch_train_loss_list = epoch_train_loss_list.copy()

                    for idx, batch_train_loss in enumerate(batch_train_loss_list):
                        tmp_epoch_train_loss_list[idx] += batch_train_loss.item()
                    ...
                    # per epoch
                    # batch_train_loss = [batch_train_loss_list[np.argmax(tmp_epoch_train_loss_list)]]
                    # per batch
                    ...
                    # selecting the last loss by default
                    batch_train_loss = batch_train_loss_list[3]
                    retrain_batch = False
                    highest_dsv_loss = np.argmax(tmp_epoch_train_loss_list)  # per epoch
                    # highest_dsv_loss = np.argmax(batch_loss_list)  # per batch

                    if highest_dsv_loss != 3:
                        # selecting the loss requiring the most to be updated
                        batch_train_loss = batch_train_loss_list[highest_dsv_loss]
                        retrain_batch = True
                    # end dsv modified 2 ######################################
                    ...
                    # print(highest_dsv_loss, retrain_batch)

                    if retrain_batch:
                        # second full pass through the batch but now employing the last loss
                        pred, true_masks, imgs, batch_train_loss_list, metrics, labels, label_names = \
                            self.training_step(batch)
                        ...
                        if self.cuda:
                            scaler.scale(batch_train_loss_list[-1]).backward(retain_graph=False)
                            nn.utils.clip_grad_value_(self.model.parameters(), 0.1)
                            scaler.step(optimizer)
                            scaler.update()
                        else:
                            batch_train_loss_list[-1].backward(retain_graph=False)
                            nn.utils.clip_grad_value_(self.model.parameters(), 0.1)
                            optimizer.step()
                    else:
                        epoch_train_loss_list = tmp_epoch_train_loss_list
