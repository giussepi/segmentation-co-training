# -*- coding: utf-8 -*-
""" nns/models/x_attention_unet_adsv.py """
...
__all__ = ['XAttentionUNet_ADSV']


class XAttentionUNet_ADSV(torch.nn.Module, InitMixin):
    """
    Attention UNet with a replaceable attention unit and alternating deep supervision
    ...
    NOTE: Use it along with ADSVModelMGR
    """
    ...
    def __init__(
        ...
        if self.dsv:
            # deep supervision ################################################
            self.dsv1 = UnetDsv(
                in_size=self.filters[3] // factor, out_size=self.n_classes, scale_factor=1,  # 8
                data_dimensions=self.data_dimensions
            )
            self.dsv2 = UnetDsv(
                in_size=self.filters[2] // factor, out_size=self.n_classes, scale_factor=1,  # 4
                data_dimensions=self.data_dimensions
            )
            self.dsv3 = UnetDsv(
                in_size=self.filters[1] // factor, out_size=self.n_classes, scale_factor=1,  # 2
                data_dimensions=self.data_dimensions
            )
            ...
            self.outc1 = OutConv(self.n_classes, self.n_classes, self.data_dimensions)
            self.outc2 = OutConv(self.n_classes, self.n_classes, self.data_dimensions)
            self.outc3 = OutConv(self.n_classes, self.n_classes, self.data_dimensions)
            self.outc4 = OutConv(self.n_classes, self.n_classes, self.data_dimensions)
        else:
    ...
    def increase_fwd_counter(self):
        if self.fwd_counter == 4:
            self.fwd_counter = 1
        else:
            self.fwd_counter += 1
    ...
    def forward(self, x):
        ...
        if self.dsv:
            ...
            if self.training:
                ...
                elif self.fwd_counter == 2:
                    logits = self.outc2(self.dsv2(d2))
                elif self.fwd_counter == 3:
                    logits = self.outc3(self.dsv3(d3))
                elif self.fwd_counter == 4:
                    logits = self.outc4(self.dsv4(d4))
                else:
                    raise RuntimeError(
                        f'fwd_counter = {self.fwd_counter}. It must be an interger in range [1,4] ')
            else:
                logits = self.outc4(self.dsv4(d4))
        else:
            logits = self.outc(d4)
