# -*- coding: utf-8 -*-
""" nns/models/x_grid_attention_unet """
...
__all__ = ['XGridAttentionUNet']


class XGridAttentionUNet(torch.nn.Module, InitMixin):
    """
    Grid Attention UNet with a replaceable attention unit

    Based on:  Source: Based on https://github.com/ozan-oktay/Attention-Gated-Networks/blob/master/models/networks/unet_grid_attention_3D.py
    ...
    def __init__(
        ...
        Kwargs:
            ...
            dsv             <bool>: Whether or not apply deep supervision. Default False
        """
        super().__init__()
        ...
        self.gating = UnetGridGatingSignal(
            self.filters[4] // factor, self.filters[3] // factor, kernel_size=1,
            batchnorm_cls=self.batchnorm_cls, data_dimensions=self.data_dimensions
        )

        # Decoder layers ######################################################
        self.up1_with_da = AttentionConvBlock(
            self.da_block_cls(self.filters[3], self.filters[3] // factor,
        ...
        self.up3_with_da = AttentionConvBlock(
            self.da_block_cls(self.filters[1], self.filters[3] // factor,
    ...
    def forward(self, x):
        ...
        d2 = self.up2_with_da(d1, x3, central_gating=F.interpolate(
            gating, size=tuple(np.array(x3.shape[2:])//2), mode=self.upsample_mode, align_corners=False))
        d3 = self.up3_with_da(d2, x2, central_gating=F.interpolate(
            gating, size=tuple(np.array(x2.shape[2:])//2), mode=self.upsample_mode, align_corners=False))
        d4 = self.up4(d3, x1)
