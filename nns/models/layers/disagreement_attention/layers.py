...
from gtorch_utils.nns.models.segmentation.unet.unet_parts import DoubleConv, Down
...
__all__ = [
    ...
    'AttentionMergingBlock', 'AttentionAEConvBlock'
]
...
class AttentionAEConvBlock(torch.nn.Module):
    """
    Attention convolutional block for XAttentionUNet using a micro AE
    ...
    def __init__(
        ...
        if self.dattentionblock.upsample:
            ...
            self.up_conv = torch.nn.Sequential(
                ...
                DoubleConv(conv_in_channels, conv_out_channels, batchnorm_cls=self.batchnorm_cls,
                           data_dimensions=self.data_dimensions)
            )
            self.maxpool_conv = torch.nn.Sequential(
                maxpoolxd(2),
                DoubleConv(
                    conv_out_channels, conv_in_channels, batchnorm_cls=self.batchnorm_cls,
                    data_dimensions=self.data_dimensions
                )
            )
        else:
