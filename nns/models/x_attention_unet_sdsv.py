# -*- coding: utf-8 -*-
""" nns/models/x_attention_unet_sdsv """
...
class XAttentionUNet_SDSV(torch.nn.Module, InitMixin):
    """
    Attention UNet with a replaceable attention unit and Selective Deep Supervision
    ...
    Note: Use it along with SDSVModelMGR
    """
    ...
    def forward(self, x):
        ...
        if self.dsv:
            ...
            dsv3 = self.dsv3(d3)
            dsv4 = self.dsv4(d4)
            logits = self.outc1(dsv1), self.outc2(dsv2), self.outc3(dsv3), self.outc4(dsv4)
        else:
            # decoder #########################################################
            d1 = self.up1_with_da(x5, x4, central_gating=gating)
            d2 = self.up2_with_da(d1, x3)
